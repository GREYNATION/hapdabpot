...
import { CinematicRendererProps, CinematicImageScene, CinematicTone, CinematicVideoScene } from "./cinematic/types";
...
const SceneImage: React.FC<{ scene: CinematicImageScene }> = ({ scene }) => {
  ...
  const fadeInFrames = scene.fadeInFrames ?? 15;
  const fadeOutFrames = scene.fadeOutFrames ?? 15;
  const fadeOutStart = Math.max(fadeInFrames, durationInFrames - fadeOutFrames);
  ...
  // Smooth cinematic slow-zoom/drift
    ...
    <AbsoluteFill style={{ backgroundColor: "#020407", opacity }}>
      <img
        ...
        style={{
          ...
          objectFit: "cover",
          transform: `scale(${scale})`,
          filter: scene.filter ?? "contrast(1.05) saturate(0.9) brightness(0.95)",
        }}
        alt=""
      />
      <AbsoluteFill
